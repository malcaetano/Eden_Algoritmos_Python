...
covP=[]
tempo=[]
V0=0.04        # velocidade da correnteza
D=1         # largura do rio
##############################################################
X=np.array([[0], [0] ])     # <--- x e y iniciais "reais"
Xe=np.array([[0], [0] ])      # <--- inicializacao das estimativas
dt=0.1                      # <--- incremento no tempo

################### Matrizes do Modelo ######################
A=np.array([[1, 4*V0*dt/D], 
            [0, 1]])   # <------- Modelo discreto de x e y

B=np.array([[0],
            [dt]])   # <------- Matriz do empuxo d motor 

C=np.identity(2)      # indica que o sensor mede as 2 variaveis x e y

du=0.07               # velocidade do barco
################### Ruidos #################################
desv_1=0.01             # < ---- perturbação do vento
desv_2=0.02 

desv_y=0.1                # ruido na medida do sensor
...
fig.grid()
fig.xlabel('tempo')
fig.ylabel('distância x(k)')
fig.title('distância x(k) e medida com ruído')
...
ax.set_ylim(-0.2,1)
fig.grid()
ax.set_xlabel('distância horizontal x(k)')
ax.set_ylabel('distância vertical y(k)')
fig.title('Trajetória do barco no rio (distância horiz, vertical)')
